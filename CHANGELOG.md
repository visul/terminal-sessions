...
### Added
- **A plain-text note per session.** One free-form note, written in a **Terminal Session Note** editor contributed to two hosts at once: a collapsed view under the session tree in the Explorer, and its own tab in the bottom panel next to Terminal. Both show the same note, so an edit in one appears in the other. The editor follows the active persistent terminal, so what is on screen is the note for the session you are typing in; clicking a row in the new **Notes** folder pins it to that session instead, which is how a stopped session's note is read without starting it or opening a tab for it. A session that has a note is marked `✎` *before* its label — long labels are ellipsised at the tail, so only the head is guaranteed to stay visible — and the panel tab picks up the same mark while the active session has one, so it is visible without leaving the Terminal tab. The **Notes** folder sits above **Favorite Sessions**, lists the sessions carrying a note most recently edited first (first line as the description, whole text in the tooltip), and is absent entirely until a note exists. Deleting is offered in the editor header, inline on a Notes row and in its right-click menu, always behind a confirmation showing the text; emptying a note deletes it too. Kill takes the note with the session, because a restored session comes back under a new name and notes are keyed by name. Stored in `~/.terminal-sessions/notes.json`, deliberately separate from the session index so hand-written text never shares a rewrite with the generated one; keyed by workspace and session name, so a rename keeps the note and two windows converge instead of overwriting each other. `terminalSessions.showNotesFolder` (default on) hides the folder.
- **The model each subagent is answering on, on its row.** Read from the agent's own transcript rather than from what the spawn asked for, and shortened to the part that matters: `opus-4.7`, `sonnet-5`, `haiku-4.5`, `sonnet-3.5` for the version-first 3.x ids. An agent spawned with a bare alias keeps the alias. The session detail row shows the version now too (`opus-4.7`, where it used to say `opus`).

### Fixed
- **Named teammates were missing from the Agents folder entirely.** Background agents are discovered by filename, and the pattern was `agent-([0-9a-zA-Z]+).jsonl` — no hyphens. Task subagents are `agent-a<16 hex>.jsonl` and matched; a named teammate is `agent-a<name-with-hyphens><16 hex>.jsonl` and did not. In the session that surfaced this, 61 of 102 agent transcripts were being skipped and the sidebar showed one agent out of three. The pruning step had the same blind spot and now keys off what the directory scan actually saw.
- **A finished agent reported how long it ran as though it were how long ago it stopped.** `done in 6m` is a correct duration that reads as an age, so a three-day-old agent looked like it had just finished. The row now says both, `6m · 3h ago`, and drops the second half while the result is still fresh.
- **An in-process teammate was marked done every time it went quiet.** The 30-second silence rule means "exited" for a Task subagent, but a teammate is merely between turns and can be messaged again. Teammates now go **idle**, with the time since they last wrote, and the folder counts read `Agents (2 idle · 1 done)`.
- **A subagent kept displaying a tool it had already finished.** The row read the last tool seen, which deliberately outlives its own `tool_result` (it is what the running state renders), so a quiet agent sat there showing a resolved tool indefinitely. It now follows the unresolved tool use.
- **An agent's type, description and model could stay missing forever.** The parse cache is keyed on the transcript's timestamp and size, but the metadata sidecar is a separate file that can be written after it — and if the transcript never changed again, the sidecar was never re-read. It is now retried until it parses.
- **The parent-idle sweep overrode subagents that were still working.** When the lead session had been quiet for two minutes, every subagent still flagged as working was rewritten, whatever its own transcript was doing. Any agent that wrote in the last 30 seconds is now left alone: the lead going quiet says nothing about an agent that wrote a moment ago.

## [0.31.1] — 2026-09-07

Review pass over 0.31.0 (four independent reviewers); nothing new to set up, but the OpenCode plugin is at hook version 3 — accept the upgrade prompt and restart `opencode`.

### Fixed
- **OpenCode events were lost on Linux hosts where `/bin/sh` is dash.** The plugin ran the forwarder through `/bin/sh`, and the forwarder is a bash script; it now runs through its own shebang.
- **OpenCode events could land out of order.** Every event used to be its own detached process, so the log followed the scheduler — a question notification arriving after the Stop that resolved it left the tab "waiting" for good. Events now leave one at a time (4s cap per forwarder run).
- **A subagent seen mid-turn was handled as a root conversation** until the server lookup answered: it could flip the pane state and left a stray `<child>.jsonl` the resume picker listed. Events for an unknown session now wait for the lookup (3s cap) and replay in order; files without a root header are no longer listed.
- **The plugin no longer announces itself with an empty session id at load**, which relabelled whatever the pane tracked before (a Claude conversation) as "OpenCode idle".
- **A stale OpenCode pid could reset another agent's live session in the same pane** (OpenCode quit without `dispose`, Claude started in that tab): every hook event now carries or clears the pid, and the dead-pid reset returns before the transcript reconciliation that used to undo it.
- **Turning the tab mark off (or shutting the extension down) left the progress glyph on every tab**: the clear path wrote only the title channel.
- **A finished turn was marked failed for talking about rate limits.** The outcome heuristic matched a bare "429" or "rate limit" anywhere in a command's output or the agent's reply; it now requires the error shape (`API Error`, `HTTP 429`, `rate limit exceeded`, `too many requests`…).
- **A turn OpenCode ended on `session.error` read as done** when the error line itself was lost; the `turn_end` flag now counts.
- **Kill & Delete Data skipped every OpenCode conversation** ("unrecognized id format") — OpenCode ids are not UUIDs.
- Resume picker: a workspace path containing `%` or `_` matched other projects' sessions (SQL `LIKE` wildcards, now escaped); the `sqlite3` probe ran in a login shell and could cache a name the extension host cannot execute (now caches the resolved path).
- Transcript writer in the plugin: a failed `mkdir`/`appendFile` dropped lines silently (now retried, bounded queue); a missing forwarder was cached as missing until restart (re-checked every 30s).
- Two file readers leaked a descriptor when the read threw; the summary reader decoded the whole buffer instead of the bytes read.
- `engines.vscode` raised to 1.98: `${progress}` (OSC 9;4) is what every tab mark rides on now, and older hosts do not render it.
- Docs: README still told you to put `${sequence}` in the tab description in two places; setting text and changelog claimed the title form shows for anyone keeping `${sequence}` — it shows only next to `${progress}`.

## [0.31.0] — 2026-09-07

### Fixed
- **The tab mark now shows on every tab, not only on tabs restored across a reload.** The state used to be written as a terminal title (OSC 2) for `${sequence}` in the tab description — and VS Code only records that title on a terminal it was not given a name for. Every tab this extension creates is named, so on a fresh tab the mark was written and never rendered; it appeared only after a window reload, when the terminals come back nameless. The state is now written as a terminal progress report (OSC 9;4) and rendered through `${progress}`, which VS Code honours on any tab: a spinner while the agent works and one mark (the `$(error)` glyph) the moment it is your turn — finished, blocked on a prompt or failed alike; the done-vs-failed guess stays on the sidebar row, where it can be wrong without costing a glance. The extension migrates a template that carries its earlier `${sequence}` to `${progress}` on its own. The richer text form (glyph + age, `terminalSessions.tabStateStyle`) is still written as a title alongside, for anyone who puts `${sequence}` back next to `${progress}` — it renders on reload-restored tabs only.
